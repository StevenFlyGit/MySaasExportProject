        ...
        if (id) {
            if (confirm("你确认要删除此条记录吗？")) {
                ...
                $.ajax({
                    ...
                    method: "get", //jQuery1.9版本前需要使用type属性来替代
                    datatype: "json",
                    success: function (result) {
                        if (result.success) {
                            location.reload();
                        } else {
                            alert("该用户绑定了角色，不可以直接删除！")
                        }
                    },
                        ...
                        <div class="form-group form-inline">
                            <div class="btn-group">
                                <button type="button" class="btn btn-default" title="新建"
                                        onclick='location.href="/system/user/toAdd.do"'><i class="fa fa-file-o"></i> 新建
                                </button>
                        ...
                        <c:forEach items="${pageInfo.list}" var="item">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <c:if test="${item.state == 1}">启用</c:if>
                                    <c:if test="${item.state != 1 and item.state != 0}">未选择</c:if>
                                </td>
                                <th class="text-center">
                                    <button type="button" class="btn bg-olive btn-xs"
                                            onclick='location.href="${ctx}/system/user/toUpdate.do?id=${item.id}"'>编辑
                                    </button>
                                </th>
                            </tr>
                        </c:forEach>
